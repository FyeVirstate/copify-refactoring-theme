...
function ProductsContent() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('copyfy_filters_products');
      if (stored) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem('copyfy_filters_products', JSON.stringify(filtersToSave));
    } catch (error) {
    ...
    minActiveAds, maxActiveAds, minTrafficGrowth, maxTrafficGrowth,
  ...
  useEffect(() => {
    const loadSavedSearches = async () => {
      try {
        const res = await fetch('/api/saved-searches?type=products');
  ...
  const resetFilters = () => {
    ...
    // Clear localStorage
    try {
      localStorage.removeItem('copyfy_filters_products');
    } catch (error) {
